...
namespace CodeStage.AntiCheat.Examples
{
	...
	public class ActTesterGui : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			logBuilder.Length = 0;
			logBuilder.AppendLine("[ACTk] ObscuredDecimal value from inspector: " + obscuredDecimal);
			logBuilder.AppendLine("[ACTk] ObscuredBool value from inspector: " + obscuredBool);
			logBuilder.AppendLine("[ACTk] ObscuredLong value from inspector: " + obscuredLong);
			logBuilder.AppendLine("[ACTk] ObscuredDouble value from inspector: " + obscuredDouble);
			logBuilder.AppendLine("[ACTk] ObscuredVector2 value from inspector: " + obscuredVector2);
			logBuilder.AppendLine("[ACTk] obscuredVector2Int value from inspector: " + obscuredVector2Int);
			logBuilder.AppendLine("[ACTk] obscuredVector3Int value from inspector: " + obscuredVector3Int);
		...
		private void OnGUI()
		{
			...
			else
			{
				...
				using (new VerticalLayout(GUI.skin.box))
				{
					...
					GUILayout.Label("Running: " + SpeedHackDetector.Instance.IsRunning + "\n<color=\"" + ((!speedHackDetected) ? "#02C85F" : "#FF4040") + "\">Detected: " + speedHackDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
					using (new GUILayout.HorizontalScope())
					{
						GUILayout.Label("<b>Time Cheating Detector</b> (updates once per 1 min by default)");
						if (GUILayout.Button("Force check", GUILayout.Width(100f)) && TimeCheatingDetector.Instance != null && !TimeCheatingDetector.Instance.IsCheckingForCheat)
						{
					...
					GUILayout.Label("Running: " + TimeCheatingDetector.Instance.IsRunning + "\n<color=\"" + ((!timeCheatingDetected) ? "#02C85F" : "#FF4040") + "\">Detected: " + timeCheatingDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
					...
					GUILayout.Label("Running: " + ObscuredCheatingDetector.Instance.IsRunning + "\n<color=\"" + ((!obscuredTypeCheatDetected) ? "#02C85F" : "#FF4040") + "\">Detected: " + obscuredTypeCheatDetected.ToString().ToLower() + "</color>");
					GUILayout.Space(10f);
