	...
	public struct ObscuredVector2
	{
		...
		public float x
		{
			get
			{
				...
				if (ObscuredCheatingDetector.ExistsAndIsRunning && fakeValueActive && Math.Abs(num - fakeValue.x) > ObscuredCheatingDetector.Instance.vector2Epsilon)
				{
		...
		public float y
		{
			get
			{
				...
				if (ObscuredCheatingDetector.ExistsAndIsRunning && fakeValueActive && Math.Abs(num - fakeValue.y) > ObscuredCheatingDetector.Instance.vector2Epsilon)
				{
					ObscuredCheatingDetector.Instance.OnCheatingDetected();
				}
				return num;
