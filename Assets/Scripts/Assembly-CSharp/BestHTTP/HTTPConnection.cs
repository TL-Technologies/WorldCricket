...
namespace BestHTTP
{
	internal sealed class HTTPConnection : ConnectionBase
	{
		...
		private void Connect()
		{
			...
			{
				BinaryWriter binaryWriter = new BinaryWriter(new WriteOnlyBufferedStream(Stream, HTTPRequest.UploadChunkSize));
				bool flag2;
				do
				{
					flag2 = false;
					...
					if (base.HasProxy && base.Proxy.Credentials != null)
					{
						...
						case AuthenticationTypes.Digest:
						{
							...
							string text2 = digest.GenerateResponseHeader(base.CurrentRequest, base.Proxy.Credentials, isProxy: true);
							if (!string.IsNullOrEmpty(text2))
							{
					...
					if ((int)HTTPManager.Logger.Level <= 1)
					{
						HTTPManager.Logger.Information("HTTPConnection", "Proxy returned - status code: " + base.CurrentRequest.ProxyResponse.StatusCode + " message: " + base.CurrentRequest.ProxyResponse.Message + " Body: " + base.CurrentRequest.ProxyResponse.DataAsText);
					}
