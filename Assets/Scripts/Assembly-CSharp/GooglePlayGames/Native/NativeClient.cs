			...
			mTokenClient.FetchTokens(silent, delegate(int result)
			{
				...
				if (flag)
				{
					if (callback != null)
					{
						...
						nativeClient.mPendingAuthCallbacks = (Action<bool, string>)Delegate.Combine(nativeClient.mPendingAuthCallbacks, callback);
					}
				...
				else
				{
					...
					{
					case 16:
						InvokeCallbackOnGameThread(callback2, data: false, "Authentication canceled");
						break;
					case 8:
						InvokeCallbackOnGameThread(callback2, data: false, "Authentication failed - developer error");
						break;
					default:
			...
			lock (AuthStateLock)
			{
				...
				case GooglePlayGames.Native.Cwrapper.Types.AuthOperation.SIGN_IN:
					...
					else
					{
						...
						mPendingAuthCallbacks = null;
						InvokeCallbackOnGameThread(callback, data: false, "Authentication failed");
					}
					break;
